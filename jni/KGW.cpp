...
/*jbyteArray JNICALL cpda(JNIEnv * env, jclass cls, jint i, jbyteArray diff, jbyteArray past)
{
    ...
    if(p && d)
    {
        ...
        convert_mp2j(env, R, &PastDifficultyAverage);
...
int KimotoGravityWell_init(JNIEnv * env, jclass cls,jlong _TargetBlocksSpacingSeconds, jlong _PastBlocksMin,
                            jlong _PastBlocksMax, double _DeviationDenominator, int _kgwFixHeight, jlong _LatestBlockTime)
{
    PastBlocksMass				= 0;
    PastRateActualSeconds		= 0;
    PastRateTargetSeconds		= 0;
    PastRateAdjustmentRatio		= double(1);
    kgwFixHeight                = _kgwFixHeight;
...
unsigned int static KimotoGravityWell_loop2(JNIEnv * env, jclass cls,jint i, jlong BlockReadingDiff, jint BlockReadingHeight, jlong BlockReadingTime, jlong BlockLastSolvedTime) {
		...
		PastRateTargetSeconds			= TargetBlocksSpacingSeconds * PastBlocksMass;
		PastRateAdjustmentRatio			= double(1);
		if(BlockReadingHeight > kgwFixHeight){
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass cls = (env)->FindClass("info/sexcoin/kgw");
